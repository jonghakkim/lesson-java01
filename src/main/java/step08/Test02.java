/* class 문법의 용도: 사용자 정의 데이터 타입 만들기 
...
public class Test02 {
  ...
  public static void main(String[] args) {
    ...
    Member s1 = new Member("홍길동", 100, 100, 100);
    s1.compute();
    
    // 생성자 호출은 필수이다. 
    //Member s2 = new Member; // 컴파일 오류!
    
    // 그 생성자 원하는 값을 반드시 파라미터로 넘겨야 한다.
    //Member s2 = new Member(); // 컴파일 오류!

    // 반드시 생성자에 정의된 순서대로 넣어야 하고 한 개의 값이라도 빠뜨려서는 안된다. 
    //Member s2 = new Member("임꺽정", 90, 90); // 컴파일 오류!
    
    Member s2 = new Member("임꺽정", 90, 90, 90);
    s2.compute();
    
    Member s3 = new Member("유관순", 80, 80, 80);
    s3.compute();
